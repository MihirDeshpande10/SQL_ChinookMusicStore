...
-- Q2. Find the top-selling tracks and top artist in the USA 
-- and identify their most famous genres.
...
round(datediff(max(date(i.invoice_date)), 
min(date(i.invoice_date))) / coalesce(count(i.invoice_id) - 1, 0), 0) 
...
select g.name as genre, sum(i.total) as total_sales, 
...
and cp.invoice_date 
between clp.last_purchase_date - interval 3 month 
...
abs(timestampdiff(month,min(invoice_date),
max(invoice_date))) as time_for_each_customer,
...
select 
cte.customer_id, 
cte.max_invoice_date, 
cte.min_invoice_date, 
cte.time_for_each_customer,
cte.total_sales,
cte.total_items,
cte.invoice_frequency,
case 
when cte.time_for_each_customer > (select average_customer_lifetime from average_time) 
